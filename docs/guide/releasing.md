...
Available bump types are `--bump minor`, `--bump major`, `--bump patch`, `--bump dev`, `--bump post`, `--bump alpha|beta|rc` (pre-release cycle), and `--bump stable`. See [Managing Versions](versions.md) for the full version lifecycle.
